...
#define charslot std::pair<int32_t, int32_t>

static std::map<charslot, charslot> charaReplaceMap = {
...
PUBLIC int UnlockCharaMods(void *pthis, int32_t chara, int32_t var)
{
	try
	{
		charslot replace = charaReplaceMap.at(std::make_pair(chara,var));
		return check_unlock(pthis, replace.first, replace.second);
	}
